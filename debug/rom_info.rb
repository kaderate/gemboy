...
HEADERS = ['FILE', 'NAME', 'TYPE', 'BANKS', 'RAM', ''].freeze
...
widths = HEADERS.each_with_index.map do |header, i|
  key = %i[file name type banks ram ok][i]
...
rows.each { |row| line.call(row.values_at(:file, :name, :type, :banks, :ram, :ok)) }
